...
class DogboneUi:
    ...
    def face_select(self ):
        faceSelect = self.inputs.addSelectionInput(
            FACE_SELECT,
            "Face",
            "Select a face to apply dogbones to all internal corner edges",
        )
        faceSelect.tooltip = "Select a face to apply dogbones to all internal corner edges\n*** Select faces by clicking on them. DO NOT DRAG SELECT! ***"
        faceSelect.addSelectionFilter("PlanarFaces")
    ...
    def table_command(self):
        table: adsk.core.CommandInput = self.inputs.addTableCommandInput(
            'tblCmd',
            'table command',
            1,
            '1'
